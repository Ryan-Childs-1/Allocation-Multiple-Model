...
def apply_v32_no_residual(df: pd.DataFrame, base_audit: pd.DataFrame, params: Dict[str,Any]|None=None) -> pd.DataFrame:
    """Apply demand/supply enhanced no-residual adjustments to base v3 predictions."""
    params=load_v32_params() if params is None else {**DEFAULT_PARAMS, **dict(params)}
    out=base_audit.copy()
    ...
    out['v3_2_adjusted_from_base']=np.where(np.nan_to_num(rounded, nan=0.0)!=base, 1, 0)
    out['v3_2_sheet_need']=sheet_need
    out['v3_2_weighted_velocity']=weighted
    out['v3_2_adjustment_reason']=np.where(obvious_over,'demand_supply_oversupply_clamp',np.where(extreme,'extreme_weak_support_clamp','base_v3_kept'))
    return out
