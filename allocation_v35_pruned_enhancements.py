...
"""
Allocation Split Expert v3.5 Feature-Pruned Site 802.

Keeps the expanded demand/supply feature layer, removes audited harmful features from the primary post-model decision layer, and keeps the Site 802 specialist path. This module is intentionally deterministic and uses only the
...
DEFAULT_PARAMS={
    # v3.5 pruned settings: keep the useful original-sheet demand/supply layer,
    # but remove the most aggressive clamp behavior that caused avoidable false negatives.
    'allocate_cap_margin_flm': 2.50,
    'review_cap_margin_flm': 2.00,
    'cap_sheet_need_mult': 1.05,
    'cap_velocity_mult': 1.10,
    'cap_consensus_max': 0.75,
    'cap_min_base_over_rec_flms': 1.50,
    'cap_reduce_to_rec_plus_flms': 2.00,
...
# v3.5 aliases used by the app/trainer. Function body is intentionally the same
# audited no-residual path, with pruned default parameters above.
def load_v35_params(path=None):
    return load_v33_params(path)

def apply_v35_pruned_no_residual(df, base_audit, params=None):
    return apply_v33_no_residual(df, base_audit, params)
